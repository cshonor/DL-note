...
> 与第4章梯度下降闭环：**`04_梯度下降_Loss与对权重求导.md`**（**∂Loss/∂w**、**`w ← w − ηg`**）、**`05_书例图解_梯度与学习率.md`**（**delta**、**α**）、**`03_误差测量与冷热学习.md`**（误差驱动）对照阅读。模块式总览与 **SGD / BGD** 对照、训练日志脚本见 **`02_多输入神经网络_图解详解_SGD与BGD.md`**。多输入前向里的 **`w_sum`** 与 **`ele_mul`** 与第3章 **`ch03_forward_propagation/03_3.5_多个输入与加权求和.md`**、**`06_3.8_单输入多输出与逐元素乘法.md`** 同名思路一致（此处 **delta 为标量**，**ele_mul** 即「标量 × 向量」）。
...
同目录 **`mult_input_one_step.py`** 与上文四步一致。多轮训练日志（**SGD / BGD**）见 **`mult_input_train_loop.py`**，说明见 **`02_多输入神经网络_图解详解_SGD与BGD.md`**。在仓库根目录执行：
