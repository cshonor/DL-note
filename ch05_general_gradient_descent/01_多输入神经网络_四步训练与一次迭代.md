...
**书中 ❶ 单输入**与下文**步骤 2**同型：**`pred → error → delta`**。例：**`input=8.5`**、**`weight=0.1`**、**`true=1`** → **`pred=0.85`**、**`delta=−0.15`**；**`error=(pred−true)²=0.0225`**，图中 **≈0.023** 为约数。更完整的脚趾例分镜见 **`ch04_gradient_descent/05_书例图解_梯度与学习率.md`** 第二节。

![❶ 单一输入：预测、误差与 delta（书中）](img/7.png)

**书中 ③ 单输入**：**`weight_delta = input * delta`**（**`8.5 × (−0.15) = −1.275`**）。与下文**步骤 3** **`ele_mul(delta, input_vec)`** 的**每个坐标**同型；书页序号与 **`ele_mul`** 多输入段落在本书中同属**第 5 章**脉络。

![③ 单一权重：权重增量 weight_delta（书中）](img/8.png)

![多输入空白网络、首场比赛前向、error/delta、ele_mul 与 weight_deltas（书中）](img/1.png)

![学习：alpha 与按 weight_deltas 更新权重（书中）](img/2.png)

![④ 多输入：ele_mul 与权重增量；⑤ 单输入：alpha 与新权重（书中合页）](img/4.png)

书中与**步骤 2**相关的另一分镜（与第二节 **img/3** 对照阅读）：
...
**书中「② 多输入：预测、误差与增量」分镜**（与篇首 **img/7**（❶ 单输入）同一 **`pred → error → delta`** 链路；**`toes[0], wlrec[0], nfans[0]`** 与 **`pred=0.86`**、**`error≈0.020`**；**delta** 代数值仍为 **−0.14**，图中增量圈有时标绝对值 **0.14**）：

![② 多输入：做出预测，计算误差与 delta（书中）](img/3.png)

---

## 三、步骤 3：每个权重的更新量（与多 **∂Loss/∂wᵢ** 同型）

**标量 delta** 与**输入向量**逐元素相乘，得到**每个权重一行**的增量（与 **`weight_delta = input * delta`** 在单输入时同构，这里是**对每个坐标各乘一遍**；篇首 **`img/8`** 书中 **③** 即单坐标一例）：
...
![⑥ 多输入：ele_mul、alpha 与逐项更新权重（书中）](img/6.png)
...
篇首 **图解** 中的 **`img/4.png`** 上半与本节 **`ele_mul(delta, input_vec)`** 同型；图中手写 **×0.14** 多为 **`|delta|`** 示意，与代码 **`delta = −0.14`** 时应在乘积上保留负号，与上文 **「数值」** 列表一致。其下半单输入 **`weight -= weight_delta * alpha`** 与 **`ch04_gradient_descent/05_书例图解_梯度与学习率.md`** 第二节脚趾例同型。
